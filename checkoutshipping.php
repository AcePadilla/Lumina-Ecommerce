...
<head>
    ...
    <link rel="stylesheet" href="checkoutship.css">
    ...
    <marquee direction="right" direction="up">
      ...
      <nav class="nav container">
         ...
         <div class="nav__menu" id="nav-menu">
          <ul class="nav__list">
            <li class="nav__item">
               <a href="homelogedin.html" class="nav__link">Home</a>
            </li>
            ...
            <div class="checkout-container">
                ...
                <div class="checkout-steps">
                   ...
                   <div class="step completed" data-step="Address">Address</div></a>
                    <div class="step active" data-step="Shipping">Shipping</div>
                    <div class="step" data-step="Payment">Payment</div>
                        ...
                        <?php
                          ...
                          // Query to fetch shipping methods from the shipmethod table
                          $sql = "SELECT shippingmethod, shippingdescription, shippingmethodprice FROM shipmethod";
                          $result = $conn->query($sql);

                          if ($result->num_rows > 0) {
                              // Start the form
                              echo '<form action="shippingmethod.php" method="POST">';
                              
                              // Loop through all rows to display each shipping method as a radio option
                              while ($row = $result->fetch_assoc()) {
                                  $method = htmlspecialchars($row['shippingmethod']);
                                  $description = htmlspecialchars($row['shippingdescription']);
                                  $price = htmlspecialchars($row['shippingmethodprice']);

                                  // Create a sanitized version of the shipping method for the ID
                                  $methodId = strtolower(str_replace(' ', '', $method));

                                  // Echo out the radio button for this shipping method
                                  echo '<div class="shipping">';
                                  echo '<input type="radio" id="'.$methodId.'" name="shippingmethod" value="'.$method.'" required>';
                                  echo '<label for="'.$methodId.'" class="shippingContent">';
                                  echo '<h3>'.$method.'</h3>';
                                  echo '<h4>'.$description.'</h4>';
                                  echo '<h5>PHP '.$price.'</h5>'; // Assuming the price is in PHP currency
                                  echo '</label>';
                                  echo '</div>';
                              }

                              // Close the form with a submit button
                              echo '<button type="submit" class="CartBtn">Order Now</button>';
                              echo '</form>';
                          } else {
